...
export const getOverallLoanStatus = (loan: Loan): 'On Track' | 'Overdue' | null => {
  ...
  // Set a fixed "today" for consistent calculations based on our project's context.
  const MOCK_TODAY = new Date('2025-06-26T12:00:00Z');

  for (const milestone of milestones) {
    const actualCompletionDateStr = loan[milestone.db_column as keyof Loan];

    if (actualCompletionDateStr) {
      // --- Logic for COMPLETED milestones ---
      ...
      // --- Logic for the FIRST PENDING milestone ---
      ...
      // Stop calculating after the first pending milestone.
      break; 
    }
  }

  // After the loop, the final cumulative variance determines the status.
