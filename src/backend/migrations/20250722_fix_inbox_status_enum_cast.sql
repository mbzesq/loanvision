-- Fix the check_missing_documents_tasks function to properly cast enum values
-- This fixes the error: invalid input value for enum inbox_status: "pending"
    ...
    LOOP
        ...
        IF days_since_added > 7 THEN
            ...
            IF NOT doc_exists THEN
                -- Only create if no active task exists (using correct enum values with explicit cast)
                IF NOT EXISTS(
