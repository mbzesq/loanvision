...
export const down = async (pool: Pool): Promise<void> => {
  await pool.query(`
    ...
    DROP TRIGGER IF EXISTS generate_inbox_external_id_trigger ON inbox_items;
    DROP TRIGGER IF EXISTS log_inbox_activity_trigger ON inbox_items;
    
    -- Drop functions
    DROP FUNCTION IF EXISTS update_inbox_items_updated_at_column();
    DROP FUNCTION IF EXISTS generate_inbox_external_id();
    DROP FUNCTION IF EXISTS log_inbox_activity();
    
    -- Drop indexes
    DROP INDEX IF EXISTS idx_inbox_items_status;
    DROP INDEX IF EXISTS idx_inbox_items_priority;
    DROP INDEX IF EXISTS idx_inbox_items_assigned_to;
    DROP INDEX IF EXISTS idx_inbox_items_created_by;
    DROP INDEX IF EXISTS idx_inbox_items_thread_id;
    DROP INDEX IF EXISTS idx_inbox_items_loan_ids;
    DROP INDEX IF EXISTS idx_inbox_items_due_date;
    DROP INDEX IF EXISTS idx_inbox_items_created_at;
    DROP INDEX IF EXISTS idx_inbox_items_category;
    DROP INDEX IF EXISTS idx_inbox_recipients_user;
    DROP INDEX IF EXISTS idx_inbox_activity_log_item;
    
    -- Drop tables in reverse dependency order
    DROP TABLE IF EXISTS inbox_activity_log CASCADE;
    DROP TABLE IF EXISTS inbox_attachments CASCADE;
    DROP TABLE IF EXISTS inbox_recipients CASCADE;
    DROP TABLE IF EXISTS inbox_items CASCADE;
    
    -- Drop custom types
    DROP TYPE IF EXISTS inbox_item_type CASCADE;
    DROP TYPE IF EXISTS inbox_priority CASCADE;
    DROP TYPE IF EXISTS inbox_status CASCADE;
    DROP TYPE IF EXISTS inbox_source CASCADE;
  `);
};